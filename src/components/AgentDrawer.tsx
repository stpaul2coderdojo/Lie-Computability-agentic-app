...
export const AgentDrawer: React.FC<AgentDrawerProps> = ({
  ...
  const [chatMessages, setChatMessages] = useState<Array<{ sender: 'user' | 'agent'; text: string; time: string }>>([
    {
      sender: 'agent',
      text: `Hello! I am the **Google Antigravity Agent**. I specialize in tensor network contraction topology, Lie algebra embeddings ($\mathfrak{so}(n), \mathfrak{su}(n), \mathfrak{sp}(2n)$), and differential geometric expressivity analysis for deep learning networks. What would you like to derive?`,
      time: 'Just now'
    }
  ]);
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-full sm:w-[480px] lg:w-[560px] bg-slate-950/95 backdrop-blur-xl border-l border-slate-800 shadow-2xl z-50 flex flex-col animate-in slide-in-from-right duration-300">
      {/* Drawer Header */}
          ...
          <div className="flex items-center p-0.5 bg-slate-950 rounded-lg border border-slate-800 text-[11px] font-mono">
            <button
              ...
              className={`px-2 py-1 rounded-md transition-all ${
                activeTab === 'proof' ? 'bg-cyan-500/20 text-cyan-300' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              Proof
            </button>
            <button
              ...
              className={`px-2 py-1 rounded-md transition-all ${
                activeTab === 'cot' ? 'bg-purple-500/20 text-purple-300' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              Steps
            </button>
            <button
              ...
              className={`px-2 py-1 rounded-md transition-all ${
                activeTab === 'chat' ? 'bg-indigo-500/20 text-indigo-300' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
              Chat
            </button>
          ...
          <div className="space-y-4 text-xs font-mono">
            <div className="p-3 rounded-xl bg-cyan-950/30 border border-cyan-500/30">
              ...
              <p className="text-slate-300 text-[11px] leading-relaxed">
                {agentResponse?.summary ||
                  `The tensor reduction graph for ${modelName} exhibits a non-abelian commutator spectrum with non-vanishing gauge holonomy along residual connections, categorizing it as a Class III Semi-Simple Lie architecture.`}
              </p>
            </div>
            ...
            <div className="p-4 rounded-xl bg-slate-900/80 border border-slate-800 space-y-3">
              <div className="flex items-center justify-between text-slate-300 font-bold">
                ...
                <span className="text-[10px] text-purple-400">Theorem 3.1</span>
              </div>

              <div className="p-3 bg-slate-950 rounded-lg border border-slate-800 text-[11px] text-purple-200 whitespace-pre-wrap leading-relaxed">
                {agentResponse?.mathematicalProof ||
`\\textbf{Theorem (Lie Complexity Bound on Contraction Manifolds)}:
            ...
            <div className="p-4 rounded-xl bg-slate-900/80 border border-slate-800 space-y-2">
              ...
              <div className="space-y-2 mt-2">
                {(agentResponse?.theoreticalInsights || [
                  'Root generator density ensures universal approximation on the compact Lie group manifold.',
                  'Grouped-Query Attention (GQA) reduces the Lie algebra rank while preserving Casimir invariant spectrum.',
                  'Residual skip loops introduce non-zero Berry phase holonomy protecting gradient flow.',
                  'Non-commutative layer commutators expand expressivity exponentially over linear architectures.'
                ]).map((insight, idx) => (
                  <div key={idx} className="flex items-start gap-2 p-2 rounded bg-slate-950/60 border border-slate-800/60 text-[11px] text-slate-300">
          ...
          <div className="space-y-3 font-mono text-xs">
            ...
            <div className="p-3 rounded-lg bg-slate-900/80 border border-slate-800 space-y-1">
              ...
              <p className="text-[11px] text-slate-300">
                Parsed tensor indices. Extracted Betti numbers β₀=1, β₁=2, Euler characteristic χ=-1. Graph diameter: 4 hops.
              </p>
            </div>
            ...
            <div className="p-3 rounded-lg bg-slate-900/80 border border-slate-800 space-y-1">
              ...
              <p className="text-[11px] text-slate-300">
                Embedded weight operators into 𝔰𝔬(n). Generated Cartan-Killing form matrix with signature (p,q)=(4,0).
              </p>
            </div>
            ...
            <div className="p-3 rounded-lg bg-slate-900/80 border border-slate-800 space-y-1">
              ...
              <p className="text-[11px] text-slate-300">
                Evaluated Frobenius commutator norms for all layer pairs. Computed Wilson loop phase shift Δθ = 1.48 rad.
              </p>
            </div>
            ...
            <div className="p-3 rounded-lg bg-slate-900/80 border border-slate-800 space-y-1">
              ...
              <p className="text-[11px] text-slate-300">
                Calculated composite Ω_Lie = 81.3 / 100. Classified architecture as Class III Semi-Simple.
              </p>
            </div>
          </div>
        )}
          ...
          <div className="flex flex-col h-full space-y-3">
            <div className="flex-1 space-y-3">
              ...
              {isSending && (
                <div className="p-3 rounded-xl bg-slate-900 border border-slate-800 text-xs font-mono text-cyan-400 flex items-center gap-2">
                  ...
                  <span>Agent reasoning over Lie algebraic structures...</span>
                </div>
              )}
            </div>
            ...
            <form onSubmit={handleSend} className="flex gap-2 pt-2">
              <input
                ...
                placeholder="Ask about Lie symmetries, Berry phases, optimal contractions..."
